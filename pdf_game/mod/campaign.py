...
from ..render_utils import white_arrow_render

from .scenes import abyss_bottom, seamus_through_small_window, the_end, BASE_MUSIC_URL
from .world import is_instinct_preventing_to_enter_village, is_instinct_preventing_to_pass_mausoleum_portal, is_instinct_preventing_to_pass_village_portal


VICTORY_POS = Checkpoint((9, 11, 5))  # end pos after beating the Empress
CHECKPOINTS = (  # intermediate positions that should be reachable through a unique path only
    ...
    Checkpoint((10, 2, 1), condition=lambda gs: gs.mp >= 2\
                                        and gs.spellbook >= 2), # Entering Templar Academy with the BURN spell
    ...
    # Checkpoint((4, 10, 14), condition=lambda gs: 'BEEN_TO_VILLAGE' in gs.hidden_triggers),  # after beating the Shadow Soul -> secret
    # Checkpoint((5, 3, 1)),  # back to Cedar Village after finding dead tree secret
    Checkpoint((8, 14, 7), condition=lambda gs: gs.armor > 1),  # Mausoleum exit before dragon, with strong armor
    Checkpoint((9, 2, 5)),                                      # Dead Walkways entrance, after beating dragon
    Checkpoint((9, 5, 5)),                                      # after passing 1st Dead Walkways arch
    Checkpoint((9, 8, 2), condition=lambda gs: 'BUCKLER' in gs.items),  # after passing 2nd Dead Walkways arch
...
# VICTORY_POS = Checkpoint((1, 4, 9)); CHECKPOINTS = (VICTORY_POS,)  # stop after beating 1st imp
# VICTORY_POS = Checkpoint((5, 3, 1)); CHECKPOINTS = (Checkpoint((1, 4, 9)), VICTORY_POS)  # stop at Cedar Village
...
def script_it():
    ...
    def _facing_dead_tree(game_view, _GameView):
        ...
        game_view.actions['LIGHT'] = _GameView(game_view.state.with_secret('DEAD_TREE')
                                                        ._replace(message=msg, treasure_id=25,
                                                                  hp=12,  # restoring HP
    ...
    # Block exit to the plains; required to win: a rest night
    ...
    def _grant_rusty_sword(game_view, _):
        game_view.state = game_view.state._replace(message="You found\nthe Saint Knight\nrusty sword!", weapon=4,
                                                   ...
                                                   music_btn_pos=Position(x=72, y=40))
    ...
    def _grant_fire_boost(game_view, _GameView):
        msg = 'You feel the fire warmth\nmaking you stronger!'
        game_view.state = game_view.state._replace(message=msg, bonus_atk=10)  # TODO: test rendering
    ...
    mapscript_add_chest((7, 13, 5), 37, _grant_fire_boost, replace=True)  # used to be: Magic Diamond (Def Up)
    ...
    mapscript_add_enemy((8, 7, 4), 'mimic', **mimic_stats, reward=RewardItem('ARMOR_PART', 40))
    ...
    mapscript_add_enemy((8, 10, 11), 'goblin',  # MUST be bribed
        category=ENEMY_CATEGORY_BEAST, bribes=('FISH',),
    ...
    mapscript_add_enemy((8, 3, 2), 'mimic', **mimic_stats, reward=RewardItem('ARMOR_PART', 41),
                                            hidden_trigger='FOUNTAIN_HINT')
    mapscript_add_enemy((8, 9, 3), 'mimic', **mimic_stats, reward=RewardItem('ARMOR_PART', 42))
    ...
    mapscript_add_enemy((9, 3, 3), 'goblin',  # deal is a red-herring, run away if fought
        ...
        category=ENEMY_CATEGORY_BEAST, bribes=('3_gold',), hp=10, rounds=(CR('', run_away=True),))

    mapscript_add_enemy((9, 3, 7), 'druid',  # deal is a red-herring, ask for mercy if fought -> restore hero HP/MP
        hp=40, rounds=(
            CR('Dagger slash', atk=7),
    ...
    mapscript_add_enemy((9, 9, 3), 'demon_seamus',
        ...
        category=enemy().ENEMY_CATEGORY_DEMON, hp=13*5-2, max_rounds=7, rounds=(
            ...
            CR('Soul sucking', mp_drain=True),
            CR('Spit frost', atk=42),
            CR('Hypnotic stare', atk=19),
        ),
        loop_frames=True,
